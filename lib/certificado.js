 ...
 * @param {Date|string|null} concluido_em_override — timestamp vindo do RETURNING
 *   do UPDATE (evita race condition: o SELECT abaixo poderia pegar concluido_em
 *   NULL se a transação anterior ainda não fosse visível em instâncias paralelas
 *   do Vercel). Se não fornecido, usa o valor do banco (comportamento antigo).
 */
async function gerarCertificadoPDF(matriculaId, funcionarioId, concluido_em_override = null) {
  ...
  // BUG CORRIGIDO: usar o timestamp passado via RETURNING do UPDATE em vez de
  // confiar no SELECT (que pode retornar NULL por race condition no Vercel).
